// Weekly Review page - for weekly habits
...
export default function WeeklyReview() {
  ...
  const { data: habits = [], isLoading: habitsLoading } = useHabitsByPeriod('weekly');
  const { entries, saveEntry, completePeriod, isSaving, isCompleting } = usePeriodEntries(weekPeriod.key);
  const { data: isCompleted } = usePeriodStatus(weekPeriod.key);
  ...
  const handleSubmit = async () => {
    try {
      ...
      showToast('success', 'Weekly review completed! 🎉');
      setTimeout(() => navigate('/'), 1000);
    } catch (error: any) {
      console.error('[WeeklyReview] Submit error:', error);
      showToast('error', error.message || 'Failed to save weekly review');
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      <header className="flex flex-col space-y-2">
        ...
        <p className="text-white/50">{weekLabel}</p>
      </header>

      <PeriodStatus
        periodKey={weekPeriod.key}
        completed={isCompleted || false}
        eligible={weekPeriod.eligible}
        onTrigger={triggerWeekly}
      />

      {(weekPeriod.eligible || isCompleted) && (
            ...
            >
              {isCompleting ? (
                ...
                <>
                  ...
                  <span>Complete Weekly Review</span>
                </>
              )}
            </button>
          )}
